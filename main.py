...
def prepare_audio_feature(music_id):
    ...
    with open(f'feature_{music_id}.npy', 'wb') as f:
        np.save(f, feature_vectors)
    return feature_vectors


def read_audio_feature(music_id):
    with open(f'feature_{music_id}.npy', 'rb') as f:
        return np.load(f)


def read_or_fallback_feature(music_id):
    if not isfile(f'feature_{music_id}.npy'):
...
def get_music_files():
    wav_path = "./wavs"
    onlyfiles = [f for f in listdir(wav_path) if isfile(join(wav_path, f))]
